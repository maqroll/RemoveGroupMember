...
public class RemoveGroupMembersMockAdminClient extends MockAdminClient {
  ...
  @Override
  public synchronized RemoveMembersFromConsumerGroupResult removeMembersFromConsumerGroup(String groupId,
      RemoveMembersFromConsumerGroupOptions options) {
    Map<MemberIdentity, Errors> res = new HashMap<>();

    options.members().stream().forEach(memberToRemove -> {
    ...
    KafkaFutureImpl<Map<MemberIdentity, Errors>> future = new KafkaFutureImpl<Map<MemberIdentity, Errors>>();
    future.complete(res);
  ...
  @Override
  public synchronized DescribeConsumerGroupsResult describeConsumerGroups(Collection<String> groupIds,
    ...
    for (String groupId : groupIds) {
      ...
      instances.values().stream().forEach(memberIdentity -> {
        members.add(new MemberDescription(memberIdentity.memberId(), Optional.of(memberIdentity.groupInstanceId()), "clientId", "host", new MemberAssignment(Collections.emptySet())));
      });

      consumerGroupDescription.complete(new ConsumerGroupDescription(groupId, true, members, "partitionAssignor", ConsumerGroupState.STABLE, null));
